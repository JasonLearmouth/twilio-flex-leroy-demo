...
import { Panel2Tabs, Panel2VoiceTabs, Panel2JobDispatchTabs } from '../../custom-components/Panel2Tabs';
...
export const componentHook = function addTabsCRMContainer(flex: typeof Flex, _manager: Flex.Manager) {
  ...
  const voice_tab_options: Flex.ContentFragmentProps = {
    if: (props: any) => {
      ...
      // When completing the task, selectedTaskSid still exist but the task in the map has been removed, so we have to check the size of it
      if (props.task) {
        return Flex.TaskHelper.isCallTask(props.task) && !Flex.TaskHelper.isInWrapupMode(props.task);
      }
      if (props.selectedTaskSid && props.tasks.size > 0) {
        const selectedTaskSid = props.tasks.get(props.selectedTaskSid);
        return Flex.TaskHelper.isCallTask(selectedTaskSid) && !Flex.TaskHelper.isInWrapupMode(selectedTaskSid);
      }
      return false;
    },
    sortOrder: -1,
  ...
  const job_dispatch_tab_options: Flex.ContentFragmentProps = {
    if: (props: any) => {
      if (props.task) {
        return props.task.attributes.taskType === 'dispatch_job';
      }
      return false;
    },
    sortOrder: -1,
  ...
  flex.CRMContainer.Content.replace(
    <Panel2JobDispatchTabs key="crm-container-job-dispatch-tabs" />,
    job_dispatch_tab_options,
  );
};
