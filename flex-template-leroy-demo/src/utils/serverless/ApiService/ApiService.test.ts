...
describe('utils/common/ApiService', () => {
  ...
  it('should provide access to the configured serverless domain', () => {
    ...
    const {
      serverless_functions_domain_leroy_demo_flex_leroy_demo,
      serverless_functions_port,
    } = custom_data;
    expect(TestService.serverlessDomain).toBe(`${serverless_functions_domain_leroy_demo_flex_leroy_demo}:${serverless_functions_port}`);
  });
